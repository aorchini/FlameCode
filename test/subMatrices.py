...
def buildMatrix(Nr,dR,beta,den,r,dFMeanDr,d2FMeanDr2,Mark,M,A,B,C,Nx,dx,tau,heatRelease,K,yMean):
    ...
    Matrix = np.zeros([Nr+Nx+M, Nr+Nx+M])
...
def M_fs(M,Nr,C,Nx,r,dFMeanDr,dx,beta):
    ...
    for ii in range (0,M):
        # AO: CHECK THE SIGNSSSSSSSSSSSSS!!!!!!!!!!!
    ...
    # checked ok. Be careful with the nonuniform finite difference coefficients
    return M_fs
...
def M_fv(Nx,Nr,r,dFMeanDr,dx,beta):
    ...
    # This assumes that Nr = Nx, and that we have f(1) = f(r=R) and v(1) =
    # v(x=0);
...
def M_sf(beta,Nr,M,tau,B,r,den,Mark,dFMeanDr,d2FMeanDr2,yMean,heatRelease):
    ...
    mu = np.ones(Nr)
    # First point (r = 1) separatley later
    mu[0] = 7./6.
    mu[1] = 23./24.
    mu[-2] = 23./24.
    mu[-1] = 7./6.
    ...
    for jj in range (0,Nr):
        for ii in range (0,Nr):
            ...
            - Mark*( D2[ii,jj]/den[ii] - 2*beta*beta*d2FMeanDr2[ii]*dFMeanDr[ii]*D1[ii,jj]/(den[ii]*den[ii]) + D1[ii,jj]/r[ii] ) )
    ...
    dq[0] +=  corrCoeff*1.0*(dFdRCorr*D1C1/np.sqrt(denCorr) +
                             - Mark*( D2C1/denCorr - 2*beta*beta*dF2dR2Corr*dFdRCorr*D1C1/(denCorr*denCorr) + D1C1/1.0 ) )

    dq[1] +=  corrCoeff*1.0*(dFdRCorr*D1C2/np.sqrt(denCorr) +
        - Mark*( D2C2/denCorr - 2*beta*beta*dF2dR2Corr*dFdRCorr*D1C2/(denCorr*denCorr) + D1C2/1.0 ) )
...
def M_vs(M,C,Nx,K,dx):
    ...
    # Be careful with the nonuniform finite difference coefficients
    return M_vs
